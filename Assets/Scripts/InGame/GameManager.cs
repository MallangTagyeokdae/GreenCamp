...
public class GameManager : MonoBehaviour
{
    ...
    public void AddClickedObject(GameObject clickedObj)
    {
        ...
        if (CheckState("InGame"))
        {
            bool isToggle = (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift)) ? true : false;

            if (isToggle)
            {
                ToggleClick(clickedObj, startIndex);
            }

            else
            {
                TryClickObject(clickedObj, startIndex);
            }
    ...
    private void ToggleClick(GameObject clickedObj, int startIndex)
    {
        if (!clickedObject.Contains(clickedObj))
        {
            TryClickObject(clickedObj, startIndex);
        ...
        TryResolveClick(clickedObj);
    }

    private void TryClickObject(GameObject clickedObj, int startIndex)
    {
    ...
    private void TryResolveClick(GameObject clickedObj)
    {
        clickedObject.Remove(clickedObj);
        clickedObj.GetComponent<Entity>().clickedEffect.SetActive(false);
        unitController.SetActiveHealthBar(clickedObj, false);
    }
    ...
    public async void LevelUpBuilding()
    {
        if (clickedObject[0].TryGetComponent(out Building building) && clickedObject.Count == 1)
        {
            if (GameStatus.instance.CanLevelUp(building, commandLevel))
            {
                ...
                if (await OrderCreate(building, building.level * 7f, cts.Token))
    ...
    public async Task<Unit> DelayUnitCreation(Building building, string unitType, Vector3 buildingPos, CancellationToken token)
    {
        ...
        {
            case "Soldier":
                progressState = await OrderCreate(building, 5.0f, token);
                break;
            case "Archer":
                progressState = await OrderCreate(building, 6.5f, token);
                break;
            case "Tanker":
                progressState = await OrderCreate(building, 8f, token);
                break;
            case "Healer":
                progressState = await OrderCreate(building, 10f, token);
                break;
            case "Scout":
                progressState = await OrderCreate(building, 3.5f, token);
                break;
    ...
    public async void CallUpgrade(Building building, int upgradeLevel, string type, int cost)
    {
        ...
        // 업그레이드 시간설정 부분
        if (await OrderCreate(building, upgradeLevel * 12f, cts.Token))
        {
            tasks.Remove(building.gameObject); // 레벨업이 완료되면 딕셔너리에서 제거해줌
